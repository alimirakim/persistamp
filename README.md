...
_(Inspired by mobile app [Loop Activity Tracker](https://play.google.com/store/apps/details?id=org.isoron.uactivities&hl=en_US&gl=US).)_
...
- **To _see a program's reward points and shop_, click the _shop icon_ at the top-right corner of a program card.**
...
> a. Log into PostgreSQL and enter its shell. Open the terminal and run this command as root user:
> `sudo su - postgres`
>
> b. Create a user with password as set up in `.env`.
> Example `.env` line:
> `DATABASE_URL=postgresql://insert_username_here:insert_password_here@localhost/insert_database_name_here`
>
> Run this command to create the user with password.
> `createuser --interactive --pwprompt`
>
> - Enter name of role (the username).
> - Enter password for role.
> - Enter password again.
> - Enter Y to make user a superuser.
>
> c. Check the user is created by entering postgres with `psql` and typing `\l` or `\list` to list users.
>
> d. Create database in postgres `createdb -O insert_username_here insert_database_name_here` or enter `psql` and create database with `CREATE DATABASE insert_database_name_here OWNER insert_username_here`.
>
> e. Check the database exists in `psql` using `\du`.
> f. Log into the database with `sudo -u postgres psql -d insert_database_name_here`. You should see `insert_database_name_here=# `. Enter `\dt` to see the database tables.
...
5. Get into your pipenv, migrate your database, seed your database, and run your flask app.
   ...
   Note: if you don't have a `/migrations` folder yet, run `flask db init`, then `flask db migrate -m "Migration name here`, then `flask db upgrade`.
